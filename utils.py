...
def _measure_mean_and_std(images):
    """
    计算一个batch中的图片每个通道像素值的均值和标准差(假设所有图片都是三通道的)
    :param images: 一个batch的所有图片
    :return: 三个通道的均值和标准差
    """
    # 每个通道的均值核标准差
    means = []
...
def get_label_dict_from_string(all_classes):
    label_dict = {}
    for idx, cls in enumerate(all_classes):
        tmp_dict = {'id': idx + 1, 'name': idx}
        label_dict[idx + 1] = tmp_dict.copy()
    return label_dict
