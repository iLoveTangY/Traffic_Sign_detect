...
def prelu(inputs):
    ...
    :return:
    """
    alphas = tf.get_variable('alpha', inputs.get_shape()[-1], initializer=tf.constant_initializer(0.0),
                             dtype=tf.float32)
...
def P_Net(inputs, label=None, bbox_target=None, training=True):
    ...
    with slim.arg_scope([slim.conv2d], normalizer_fn=None, weights_initializer=slim.xavier_initializer(),
                        activation_fn=prelu, biases_initializer=tf.zeros_initializer(), padding='valid'):
...
def R_Net(inputs, label=None, bbox_target=None, training=True):
    ...
    with slim.arg_scope([slim.conv2d], normalizer_fn=None, weights_initializer=slim.xavier_initializer(),
        ...
        net = slim.fully_connected(net, 128, normalizer_fn=None, biases_initializer=tf.zeros_initializer(),
                                   activation_fn=prelu, scope='fc1')

        net = slim.flatten(net)
        cls_prob = slim.fully_connected(net, 1, normalizer_fn=None, biases_initializer=tf.zeros_initializer(),
                                        activation_fn=tf.sigmoid, scope='fc2')
        bbox_pred = slim.fully_connected(net, 4, normalizer_fn=None, biases_initializer=tf.zeros_initializer(),
...
def O_Net(inputs, label=None, bbox_target=None, training=True):
    ...
    with slim.arg_scope([slim.conv2d], normalizer_fn=None, weights_initializer=slim.xavier_initializer(),
        ...
        net = slim.fully_connected(net, 256, normalizer_fn=None, biases_initializer=tf.zeros_initializer(),
                                   activation_fn=prelu, scope='fc1')
