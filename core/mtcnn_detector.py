            ...
            while min(current_height, current_width) > net_size:
                ...
                for current_y in current_y_list:
                    for current_x in current_x_list:
                        ...
                        current_rectangle = [int(w * float(current_x) / current_width),
                                             int(h * float(current_y) / current_height),
                                             int(w * float(current_x) / current_width) + int(
                                                 w * float(net_size) / current_width),
                                             int(h * float(current_y) / current_height) + int(
                                                 w * float(net_size) / current_width),
                                             0.0]
...
face candidates:%d, current batch_size:%d"%(num_boxes, batch_size)
            ...
            while min(current_height, current_width) > net_size:
                ...
                boxes = generate_bbox(cls_map[0, :, :], reg, current_scale, self.thresh[0])
...
face candidates:%d, current batch_size:%d"%(num_boxes, batch_size)
    ...
    def detect(self, img):
        ...
        if self.onet_detector:
            boxes, boxes_c = self.detect_onet(img, boxes_c)
            if boxes_c is None:
                return np.array([])
    ...
    def detect_face(self, imdb, test_data, vis):
        ...
        for databatch in test_data:
            ...
            if self.onet_detector:
                ...
                t = time.time()
                print(
                    "time cost " + '{:.3f}'.format(t1 + t2 + t3) + '  pnet {:.3f}  rnet {:.3f}  onet {:.3f}'.format(t1,
                                                                                                                    t2,
                                                                                                                    t3))
